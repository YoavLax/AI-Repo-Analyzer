...
class UrllibFetcher:
    ...
    def _open(self, url: str, accept: str):
        ...
        headers = {"User-Agent": "codecompass", "Accept": accept}
    ...
    def get_raw(self, url: str) -> bytes:
        with self._open(url, "*/*") as response:
            return self._read_capped(response, MAX_FILE_BYTES)
...
def _map_http_error(exc: urllib.error.HTTPError, remote: RemoteRepo) -> IngestError:
    if exc.code == 404:
        return IngestError(
            ...
            f"For private repositories, self-host CodeCompass and use local-path mode.",
            404,
        )
...
def fetch_snapshot(
    ...
    if tree_data.get("truncated"):
        raise IngestError(
            ...
            "Self-host CodeCompass and analyze it via local-path mode.",
            413,
        )
    ...
    if len(selected) > MAX_FETCH_FILES:
        raise IngestError(
            ...
            f"Self-host CodeCompass and analyze it via local-path mode.",
            413,
        )
